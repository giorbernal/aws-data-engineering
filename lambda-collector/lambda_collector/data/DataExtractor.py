...
class DataExtractor:
    ...
    def process(self):
        try:
            ...
            joined_df['FECHA']=joined_df[['ANO', 'MES', 'DIA']].apply(lambda x: str.join('-', [str(x['ANO']), str(x['MES']).zfill(2), str(x['DIA']).zfill(2)])[0:10], axis=1)
    ...
    def __adapt_dynamo_data__(self, df, value_columns):
        ...
        melt_df['FECHA'] = melt_df[['FECHA', 'HORA_VALIDEZ']].apply(lambda x: '-'.join([str(x[0]), str(int(str(x[1])[-2:])-1).zfill(2)])[0:10], axis=1)
